...
const localList = require("../../data/projectList")
const localRes = require("../../data/projectRes")
Page({
   ...
   */
  onLoad({ query }: any) {
    const projectRes = localRes.data
    const { index, id, title }: any = query ? JSON.parse(query) : {}
    this.setData({
      title: title,
      sum: localList.data.length,
      index: Number(index),
      answer: projectRes[id]
    })
    this.setCurrentProject(this.data.index)
    setWatcher(this)
  },
  ...
  watch: {
    // 需要监听的字段
    index(val: any) {
      this.setCurrentProject(val)
    }
  },

  setCurrentProject(val: any) {
    if (val === 0) {
  ...
  onShareAppMessage() {
    ...
    return{
      ...
      path: `/pages/project-res/project-res?query=${JSON.stringify(query)}`
    }
  }
})